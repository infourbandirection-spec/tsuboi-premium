...
3. **認証システム**
   ...
   - ログアウト機能
   - 未認証時のアクセス制御
   - デフォルト認証情報: `urbandirection / urbandirection`
...
### 現在のデータ状態（2026-03-05時点）
- **総応募件数**: 0件（本番運用開始前）
- **総応募冊数**: 0冊
- **発行上限**: 1490冊
- **現在フェーズ**: Phase 1（一畳屋ショールーム）
- **抽選状態**: 未実行
- **受付状態**: 受付中
...
### 本番運用リセット
テストデータを削除して本番運用を開始する場合は、以下のドキュメントを参照してください：
- **SQLファイル**: [`PRODUCTION_RESET.sql`](./PRODUCTION_RESET.sql)
- **実行手順書**: [`PRODUCTION_RESET_GUIDE.md`](./PRODUCTION_RESET_GUIDE.md)
...
2. ログイン情報入力（ユーザー名: `urbandirection` / パスワード: `urbandirection`）
3. ログイン成功後、管理画面へ自動遷移
4. **ダッシュボード**: 全体状況を確認
...
**デモシナリオ**:
1. **動的冊数制限** - 残り10冊で最大3冊、残り2冊で最大2冊選択可能
2. **在庫少量警告** - 残り3冊未満で警告表示
3. **在庫上限到達** - 予約数が上限に達すると自動的に受付停止
...
- ✅ **冊数上限変更**（6冊→3冊に変更）
  - フォーム選択: 1～3冊まで選択可能
  - データベース制約: CHECK (quantity BETWEEN 1 AND 3)
  - 警告表示更新: 残り3冊未満で警告表示
- ✅ **販売店舗変更**（「坪井繁栄会（複数店舗）」→「一畳屋ショールーム（単一店舗）」）
  - 店舗名: 一畳屋ショールーム
  - 住所: 熊本県熊本市中央区坪井5丁目2-27
  - ブランド名は「坪井繁栄会」のまま維持
- ✅ **購入場所の統一**（フォーム、抽選結果、メールテンプレート）
- ✅ **データベース更新**（店舗マスターを一畳屋ショールームに変更）
- ✅ **管理者認証情報変更**（ユーザー名/パスワード: urbandirection）

### v2.3.0 (2026-03-04)
- ✅ **ブランド名変更**（「パスート24」→「坪井繁栄会」）
- ✅ **店舗統合**（5店舗→3店舗に整理）
  - 削除: 坪井繁栄会辛島公園、熊本市辛島公園地下駐車場
  - 継続: 上通、銀座プレス、熊本中央
- ✅ **利用条件簡素化**（「立体駐車場および地下駐車場」→「一部店舗を除く」）
