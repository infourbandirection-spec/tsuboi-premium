...
app.get('/lookup', (c) => {
  return c.html(`
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>応募照会 - プレミアム引換券応募フォーム</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
        <style>
          /* カレンダーボタンを非表示（全ブラウザ対応） */
...
// フェーズ2の購入時間を初期化（フェーズ1と同じ設定をコピー）
app.post('/api/admin/pickup-time-slots/init-phase2', async (c) => {
  try {
    const db = c.env.DB
    
    // すでにフェーズ2の時間スロットが存在するかチェック
    const existingPhase2 = await db.prepare(`
      SELECT COUNT(*) as count FROM pickup_time_slots WHERE phase = 2
    `).first() as any
    
    if (existingPhase2 && existingPhase2.count > 0) {
      return c.json({
        success: false,
        error: 'フェーズ2の購入時間は既に登録されています'
      }, 400)
    }
    
    // フェーズ1の時間スロットを取得
    const phase1Slots = await db.prepare(`
      SELECT time_slot, display_label, is_active, display_order
      FROM pickup_time_slots
      WHERE phase = 1
      ORDER BY display_order
    `).all()
    
    if (!phase1Slots.results || phase1Slots.results.length === 0) {
      return c.json({
        success: false,
        error: 'フェーズ1の購入時間が見つかりません'
      }, 400)
    }
    
    // フェーズ2用に同じ時間スロットを登録
    let insertedCount = 0
    for (const slot of phase1Slots.results) {
      ...
      `).bind(
        slotData.time_slot,
        slotData.display_label,
        slotData.is_active,
        slotData.display_order
      ).run()
      insertedCount++
    }
    
    console.log(`✅ Phase 2 time slots initialized: ${insertedCount} slots copied from Phase 1`)
    
    return c.json({
      success: true,
      message: `フェーズ2の購入時間を初期化しました（${insertedCount}件）`,
      count: insertedCount
    })
  } catch (error: any) {
    logSecureError('InitPhase2TimeSlots', error)
    return c.json({
      success: false,
